...
def gasnetwork_nlp(P_time0, Q_time0, P_initialnode, Q_initialnode, eps, Edgesfile):
    ...
    number_of_edges_without_slack_edge = df.shape[0] - 1
    ...
    for edge in range(0, number_of_edges_without_slack_edge):
        ...
        if edge in starting_edges:
            ...
            for j in range(1,n):
                ...
                ubw += [P_time0[edge,j]] + [100]*(m-1)#[+cas.inf]*(m-1)
            ...
            for j in range(1,n):
                ...
                ubw += [Q_time0[edge,j]] + [200]*(m-1) #[+cas.inf]*(m-1)
        else:
            # go row-wise through P
            for j in range(0,n):
                w0 += [P_time0[edge,j]] + [60]*(m-1)
    ...
    end_edge = get_end_edge_in_network(df) # edge 6
...
if __name__ == '__main__':
     ...
     options = {'ipopt': {'tol': 1e-4, 'max_iter': 100}} # nächstes mal: 100
